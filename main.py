...
def seed():
    uid = new_id()
    users["3"] = {"id": uid, "email": "demo@quiz.com", "password_hash": hash_pw("1")}
    qid = new_id()
...
def _send_next_question(sid):
    ...
    print(f"[QUESTION] Session={sid} advancing to question index={idx} (total={len(quiz['questions'])})")
    if idx >= len(quiz["questions"]):
        print(f"[QUESTION] No more questions — ending game for session={sid}")
        return _end_game(sid)
    session["current_question_index"] = idx
    session["question_start_time"] = time.time()
    q = quiz["questions"][idx]
    print(f"[QUESTION] Broadcasting game:question_started id={q['id']} timeLimit={q['timeLimitSeconds']}s")
...
def _end_question_logic(sid):
    session = sessions.get(sid)
    if not session:
        print(f"[END-QUESTION-LOGIC] Session={sid} not found.")
        return None
    ...
    if idx < 0 or idx >= len(quiz["questions"]):
        print(f"[END-QUESTION-LOGIC] Invalid question index={idx} for session={sid}.")
        return None
    ...
    print(f"[END-QUESTION-LOGIC] session={sid} q={q['id']} answered={total_answers}/{total_participants}")
    ...
    # Broadcast the end of the question
    print(f"[END-QUESTION-LOGIC] Broadcasting game:question_ended for session={sid}")
    ...
    # Broadcast the updating leaderboard after the question
    leaderboard = _build_leaderboard(session)
    broadcast(sid, "game:leaderboard", {"leaderboard": leaderboard})
    
    return {"stats": {"totalAnswers": total_answers}, "leaderboard": leaderboard}
...
def end_question(sid):
    ...
    print(f"[END-QUESTION] Called for session={sid} by host={g.user_id}")
    result = _end_question_logic(sid)
    if not result:
        print(f"[END-QUESTION] _end_question_logic returned None — no active question?")
        return err(400, "BAD_REQUEST", "No active question")
    
    print(f"[END-QUESTION] Done. totalAnswers={result['stats']['totalAnswers']}")
...
def _perform_end_question(sid):
    """Internal helper to finalize a question and broadcast results."""
    session = sessions.get(sid)
    ...
    by_option = {o["id"]: 0 for o in q["answerOptions"]}
    ...
    broadcast(sid, "game:question_ended", {
        "questionId": q["id"],
        "correctOptionIds": list(correct_ids),
        "stats": {"totalAnswers": sum(by_option.values()), "byOption": by_option}
    })
    
    # 2. Broadcast Updated Leaderboard
    lb = _build_leaderboard(session)
    broadcast(sid, "game:leaderboard", {"leaderboard": lb})
    return lb
...
def submit_answer():
    ...
    q = quiz["questions"][idx]
    body = request.get_json(silent=True) or {}
    option_id = body.get("optionId")  # None means timeout / no answer
    
    print(f"[ANSWER] participant={g.participant_id} question={q['id']} optionId={option_id}")

    if q["id"] in participant["answers"]:
        print(f"[ANSWER] Already answered — rejecting duplicate from participant={g.participant_id}")
    ...
    print(f"[ANSWER] is_correct={is_correct} elapsed={elapsed:.1f}s points_earned={points_earned} total_score={participant['score']}")
    ...
    # Check if all participants have now answered — auto-end question if so
    total_participants = len(session["participants"])
    total_answers = sum(1 for p in session["participants"].values() if q["id"] in p["answers"])
    print(f"[ANSWER] Progress: {total_answers}/{total_participants} participants answered")
    
    if total_participants > 0 and total_answers >= total_participants:
        print(f"[ANSWER] All participants answered — auto-ending question for session={g.session_id}")
        _end_question_logic(g.session_id)
